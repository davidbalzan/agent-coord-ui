...
Persist learnings to `~/.claude/knowledge/` and sync to Agent Coord UI repo.
...
6. **Sync to Agent Coord UI** (if configured)
   - Copy knowledge directory to local Agent Coord UI clone
   - Commit and push changes
   - Agent Coord UI path: auto-detected via `$(git rev-parse --show-toplevel)` or `$AGENT_COORD_UI_PATH` env var

7. **Confirm to user**
   - Show what was saved and where
   - Indicate if Agent Coord UI sync succeeded
...
After saving locally, sync to Agent Coord UI if inside the repo:

```bash
# Detect repo root (works from any subdirectory)
AGENT_COORD_UI_ROOT="${AGENT_COORD_UI_PATH:-$(git rev-parse --show-toplevel 2>/dev/null)}"

if [ -n "$AGENT_COORD_UI_ROOT" ]; then
  # Copy knowledge to Agent Coord UI
  cp -r ~/.claude/knowledge "$AGENT_COORD_UI_ROOT/.claude/"

  # Commit and push
  cd "$AGENT_COORD_UI_ROOT"
